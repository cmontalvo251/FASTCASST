function dxdt = Derivatives(xstate,t,controller)
global da de
%% Aircraft Properties %%
% run airplane_calcs%%% Aircraft properties
% fid = fopen('FoamFlyer.txt')
...
% Ixx = 0.001; %kg*m^2 (from xflyer sim)
% Iyy = 0.08;
% Izz = 0.009;
...
Lift = C_L*Qa*S;
Drag = C_D*Qa*S;
% T0 = 5.0882;  %Apprentice
% T0 = 0.889644; %(Newtons or 0.2 lb_f)
Thrust = T0 + C_x_dT*dT;

%--------------------------------------------------------------------%
% C_L = C_L_alpha*alpha + C_L_de*de;
% C_D = C_D_0 + C_D_alpha*(alpha^2) + C_D_u*u;
% 
% C_l = C_l_p*phat + C_l_r*rhat + C_l_da*da;
% C_m = C_m_0 + C_m_alpha*alpha + C_m_q*qhat + C_m_de*de + C_m_u*u;
% C_n = C_n_p*phat + C_n_r*rhat + C_n_da*da;
% C_x = 0;
% C_y = C_y_p*phat + C_y_r*rhat;
% C_z = 0;
% 
% Lift = C_L*Qa*S;
% Drag = C_D*Qa*S;
