...
while i < len(t)-1:
	...
	if (time.monotonic() - timeStart) > 1.0:
		...
		GND.clearwindow()
		telemetry_packet[0] = t[i]
		telemetry_packet[1] = roll[i]
		telemetry_packet[2] = pitch[i]
		telemetry_packet[3] = yaw[i]
		telemetry_packet[4] = latitude[i]
		telemetry_packet[5] = longitude[i]
