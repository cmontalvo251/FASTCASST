...
void Autopilot::loop(float rollrc, float pitchrc, float throttle, float elapsedTime,float oh_shit){
  ...
  if (oh_shit > 1800) {
    ////Holy crap. Go into a spiral please
  ...
  if (VELOCITYCONTROL) {
    signals[0] = mewt; //Send to motor1
    signals[1] = mewt; //Send to motor2
  } else {
    signals[0] = throttle;
    signals[1] = throttle;
  }
  signals[2] = aileron_signal;
  signals[3] = elevator_signal;
  signals[4] = rudder_signal;
}
