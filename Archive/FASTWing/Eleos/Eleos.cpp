...
//#define USEUSB
#define NOPANIC
#define DIFFERENTIAL_Left -1.0
#define DIFFERENTIAL_Right 1.0
...
int main(int argc, char* argv[]){
  ...
  OUTPUT.zeros(27,1,"OUTPUT MATRIX");
  /////////////////////////////////////////////////
  ...
  rcout.PWMOUTPUTS[2] = 2; //Throttle - Motor1
  rcout.PWMOUTPUTS[3] = 3; //Throttle - Motor2
  rcout.PWMOUTPUTS[4] = 4; //Yaw
  ...
  while (1) {
	...
	double pm_rudder = (yawrc - RUDDER_SERVO_MID)/double(RUDDER_SERVO_MID-RUDDER_SERVO_MIN);
        MOTORSOUT[0] = throttle + DIFFERENTIAL_Left*pm_rudder*(SERVO_MAX-SERVO_MID); //For Motor1
	//Check for saturation
	if (MOTORSOUT[0] < SERVO_MIN) {
	  MOTORSOUT[0] = SERVO_MIN; 
	}
	if (MOTORSOUT[0] > SERVO_MAX) {
	  MOTORSOUT[0] = SERVO_MAX;
	}
	MOTORSOUT[1] = throttle + DIFFERENTIAL_Right*pm_rudder*(SERVO_MAX-SERVO_MID); //For Motor2
	//Check for saturation
	if (MOTORSOUT[1] < SERVO_MIN) {
          MOTORSOUT[1] = SERVO_MIN; 
	}
	if (MOTORSOUT[1] > SERVO_MAX) {
	  MOTORSOUT[1] = SERVO_MAX;
	}
        MOTORSOUT[2] = rollrc;
        MOTORSOUT[3] = pitchrc;
        MOTORSOUT[4] = yawrc;
	...
	printf("Unmounting USB! \n");
      ...
      MOTORSOUT[1] = THROTTLE_MIN;
      MOTORSOUT[2] = SERVO_MID;
      MOTORSOUT[3] = SERVO_MID;
      MOTORSOUT[4] = SERVO_MID;
      }  //end ARM SWITCH
    ///////////////////////////////////////////////
    ...
    if ((((currentTime - dataTime)/1000000.0) > LOGPERIOD) && (FILEOPEN == 1)) {
      ...
      OUTPUT.set(14,1,MOTORSOUT[0]); //Throttle - MOTOR1
      OUTPUT.set(15,1,MOTORSOUT[1]); //Throttle - MOTOR2
      OUTPUT.set(16,1,MOTORSOUT[2]); //Rollrc
      OUTPUT.set(17,1,MOTORSOUT[3]); //Pitchrc
      OUTPUT.set(18,1,MOTORSOUT[4]); //Yawrc
      ...
      OUTPUT.set(23,1,control.pitots.windspeed[0]);
      OUTPUT.set(24,1,control.pitots.windspeed[1]);
      OUTPUT.set(25,1,control.pitots.windspeed_filtered[0]);
      OUTPUT.set(26,1,control.pitots.windspeed_filtered[1]);
      OUTPUT.set(27,1,control.barotemp.Altitude);
