...
#define ORIGINLAT 30.691756
#define ORIGINLON -88.174875
...
void loop() {
  ...
  if (GPSUpdate == true) { //this variable will change from 0 to 1 when we get new GPS coordinates.
      ...
      xy[1] = (longitudeDegrees-ORIGINLON)*GPSVAL*cos(ORIGINLAT*PI/180.0);
 
      //Use the new value and the previous of xy to compute psi
      if ((abs(xy[1]-xy_prev[1]) > 1) || (abs(xy[0]-xy_prev[0]) > 1)) { //Might need to change to get this to work properly //Or add a filter but we're running out of bytes
...
//  }
  ...
  //This routine takes care of the wrapping nature of atan2
    ...
    //Middle is 94
    //FullLeft is 45
    //Make gain no bigger than -15.6
    auto_steer = 10*delpsi+94; //Need to change 0.1 to something else and tune this
    //auto_steer = map(steerRight,servo0,servo180,0,180);
    servoSteer.write(auto_steer);
  }
  
  if(pulseMap_thrust < 60 && pulseMap_thrust > 1){
...
//   Serial.print(autopilot); 
    ...
    logfile.print(latitudeDegrees,8); logfile.print(" "); 
    logfile.print(longitudeDegrees,8);logfile.print(" "); 
