...
loggersil:
	make all TYPE="SIL" EXECUTABLE="loggersil.exe" MAIN="src/logger.cpp"

##Logger is on RPI but all it does is take data
logger:
	make all TYPE="AUTO" EXECUTABLE="logger.cpp" PLATFORM="RPI" MAIN="src/logger.cpp" WIRINGPI="-lwiringpi"

#Demo sil is on DESKTOP but only does RCIN>IMU>CONTROL>RCOUT
demosil:
	make all TYPE="SIL" EXECUTABLE="demosil.exe" MAIN="src/demo.cpp"

#Demo is demosil but on rpi
demo:
	make all TYPE="AUTO" EXECUTABLE="demo.exe" PLATFORM="RPI" RX="FLYSKY" MAIN="src/demo.cpp" WIRINGPI="-lwiringpi"
