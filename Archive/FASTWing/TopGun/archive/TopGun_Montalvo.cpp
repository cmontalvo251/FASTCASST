...
void ControlLoop(double elapsedTime) {

  //Call the PIDCONTROL ROUTINE HERE -- Need to send it sensor information
  //void Aerodynamics::PIDCONTROL(float u,float z,float zdot,float theta,float q,float phi,float p,float r) {
  float u,z,zdot,theta,q,phi,p,r;
  aero.PIDCONTROL(u,z,zdot,theta,q,phi,p,r);
  ...
  for (int i = 0;i<=4;i++) {
    ...
    inSIG[i] = aero.controls.get(i+1,1);
    MOTORSOUT_[i] = MOTORSOUT[i];
    MOTORSOUT[i] = (inSIG[i]+inSIG_[i]-MOTORSOUT_[i]*(1-alfa))/(alfa+1);
  }

  //Saturation Blocks -- Needs to be changed for servos and throttle
  for (int i = 0;i<=8;i++) {
...
int main(int argc, char *argv[])
{
  ...
  rcin.RXCHANNELS[5] = 4; //configuration switch
  rcout.num_motors = 4;
  ...
  while (true)
	...
	thrust_desired = ((1.0)/((double)THROTTLE_MAX-(double)THROTTLE_MIN))*(throttle-THROTTLE_MIN);
      }
	...
	ControlLoop(elapsedTime);
	/////////////////////////////////////////
	#ifdef LOGDATA
	if (FILEOPEN == 0) {
	  //The file is not open so open it
	  logger.findfile("Data_Files/output");
	  logger.open();
