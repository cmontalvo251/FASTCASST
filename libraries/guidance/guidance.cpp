...
void controller::loop(double currentTime,int rx_array[],MATLAB sense_matrix) {
  ...
  double throttle_rx = rx_array[0];
  double aileron_rx = rx_array[1];
  double elevator_rx = rx_array[2];
  double rudder_rx = rx_array[3];
  double arm_switch_rx = rx_array[4];
  int icontrol = 0;

  //Check for user controlled
  if (CONTROLLER_FLAG < 0) {
    if (arm_switch_rx > STICK_MID) {
  ...
  //Guidance Control Cases
  // 0 = pass receiver signals through
  // 1 = send roll,pitch,yaw commands and pass through throttle
  // 2 = send roll,pitch,yaw,throttle commands based on altitude
  
  //Initialize commands
  roll_command = -99;
  pitch_command = -99;
  yaw_rate_command = -99;
  altitude_command = 100; //Hardcoded to 100 but eventually need to have this update via telemetry
  ...
  case 0:
    throttle = throttle_rx;
    aileron = aileron_rx;
    elevator = elevator_rx;
    rudder = rudder_rx;
    arm_switch = arm_switch_rx;
    break;
  }
