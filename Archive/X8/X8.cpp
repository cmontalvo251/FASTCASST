...
///MAIN LOOP RATES
#define RXPERIOD 0.1
#define LOGPERIOD 0.1 //changed from 0.02 on 3/11/2020
...
int main(int argc, char* argv[]) {
  ...
  ///////////Setup RX////////////////////////////////////////
  rcin.NUMRX = 8;
  rcin.MOVELEVEL = 10; //even with sticks centered you should get about 0-7 pwm of noise problem is this noise continues even with transmitter off
  rcin.RESETTIME = 5; //You need at least 5 seconds so if you lose control you have 5 seconds to regain control
  ...
  while (1) {
    ...
    //This command will spin an individual motor to make sure each motor is wired correctly.
    //rcout.set_duty_cycle(0,1100);
    //rcout.set_duty_cycle(4,1100);
    ...
    if ((currentTime - printTime) / 1000000.0 > DEBUGPERIOD) { //print at 20 Hz
      ...
      //Rec Signals
      printf("%d %d %d %d %d %d %d (%d) ",throttle,rollrc,pitchrc,yawrc,configrc,arm_switch,pusherpwm,SIGNALSOFF);
      //Rec Commands
