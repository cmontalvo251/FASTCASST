function dxdt = metaDerive(xstate,t,controller,n)
...
%% Collision/Magnetic Forces and Moments
delx = xstate(1,2) - xstate(1,1);
dely = xstate(2,2) - xstate(2,1) - b;
delz = xstate(3,2) - xstate(3,1);
delydot = xstate(8,2) - xstate(8,1);
...
delphi = xstate(4,2) - xstate(4,1);
deltheta = xstate(5,2) - xstate(5,1);
delpsi = xstate(6,2) - xstate(6,1);
delp = xstate(10,2) - xstate(10,1);

kphi = 1;
cphi = 0;
if abs(delphi) <= 5*pi/180
    Lmag = kphi*delphi + cphi*delp;
else
    Lmag = 0;
end
if abs(deltheta) <= 5*pi/180
    Mmag = 4*deltheta;
else
    Mmag = 0;
end
if abs(delpsi) <= 5*pi/180
    Nmag = 4*delpsi;
else
    Nmag = 0;
end
...
dxdt = zeros(12,2);
dxdt(:,n) = [xyzdot;ptpdot;uvwdot;pqrdot];